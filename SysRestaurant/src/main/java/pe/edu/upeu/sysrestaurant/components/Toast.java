...
public class Toast {
    ...
    public static void showToast(Stage ownerStage, String message, int durationInMillis, double x, double y) {
        ...
        // AÃ±adir la etiqueta al Popup
        StackPane pane = new StackPane(label);
        pane.setPadding(new Insets(10));
        pane.setAlignment(Pos.CENTER);
        popup.getContent().add(pane);

        // Mostrar el Popup en la posiciÃ³n personalizada (x, y)
        popup.show(ownerStage, x, y);

        // Crear una animaciÃ³n para el toast (fade in -> esperar -> fade out)
